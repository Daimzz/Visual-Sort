const sortBoxBubble = document.getElementById("bubble");
const sortBoxChoice = document.getElementById("choice");

const inputNumber = document.querySelector(".inputNumber");

// Получаем значение инпута и формируем колонки в соответствии с введенными данными

inputNumber.addEventListener("change", function () {
  const columnCount = !!inputNumber.value ? +inputNumber.value : 35;
  NUMBERS_COUNT = columnCount;

  if (sortBoxBubble.querySelector(".start-btn_reload")) {
    sortBoxBubble
      .querySelector(".start-btn_reload")
      .addEventListener("click", function () {
        // Передаем в колбек-функцию параметры
        if (sortBoxBubble.querySelector(".chart-box")) {
          sortBoxBubble.querySelector(".chart-box").remove();
          sortBoxBubble.querySelector(".info-box").remove();
          createSortBox(
  ...
  if (sortBoxChoice.querySelector(".start-btn_reload")) {
    sortBoxChoice
      .querySelector(".start-btn_reload")
      .addEventListener("click", function () {
        // Передаем в колбек-функцию параметры
        if (sortBoxChoice.querySelector(".chart-box")) {
          sortBoxChoice.querySelector(".chart-box").remove();
          sortBoxChoice.querySelector(".info-box").remove();
          createSortBox(
...
function createSortBox(container, sortFunction) {
  ...
  const chartBox = document.createElement("div"),
    ...
    startBtnReload = document.createElement("button"),
  ...
  startBtnReload.classList.add("start-btn", "start-btn_reload");
  ...
  //Создание графика и перетасовка колонок после нажатия на кнопку "Перезапуск"
